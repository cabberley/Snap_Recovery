...
- [Azure Disk Snapshots vs Recovery Point Collections](#azure-disk-snapshots-vs-recovery-point-collections)
  ...
  - [Working with Recovery Points: CLI Examples](#working-with-recovery-points-cli-examples)
    - [Create a recovery point collection](#create-a-recovery-point-collection)
    - [Create a crash-consistent recovery point](#create-a-crash-consistent-recovery-point)
    - [Create an application-consistent recovery point](#create-an-application-consistent-recovery-point)
    - [List recovery points in a collection](#list-recovery-points-in-a-collection)
    - [Create managed disks from a recovery point](#create-managed-disks-from-a-recovery-point)
...
## Working with Recovery Points: CLI Examples

### Create a recovery point collection

A collection is a container tied to a specific VM. Create it once, then add recovery points to it:

```bash
# Create a recovery point collection for the VM
az restore-point collection create \
  --subscription "00000000-0000-0000-0000-000000000000" \
  --resource-group "iris-prod-rg" \
  --collection-name "iris-prod-vm-rpc" \
  --source-id "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/iris-prod-rg/providers/Microsoft.Compute/virtualMachines/iris-prod-vm"
```

### Create a crash-consistent recovery point

This captures all disks atomically without any application freeze:
...
az restore-point create \
  ...
  --resource-group "iris-prod-rg" \
  --collection-name "iris-prod-vm-rpc" \
  --name "rp-$(date -u +%Y%m%d%H%M%S)" \
  --consistency-mode CrashConsistent
```

That single command snapshots **every disk** on the VM — OS disk and all data disks — at the same instant.

### Create an application-consistent recovery point

If you need the VM guest agent to quiesce the application before capture:
...
az restore-point create \
  ...
  --resource-group "iris-prod-rg" \
  --collection-name "iris-prod-vm-rpc" \
  --name "rp-appconsistent-$(date -u +%Y%m%d%H%M%S)" \
  --consistency-mode ApplicationConsistent
```

### List recovery points in a collection
...
az restore-point collection show \
  ...
  --collection-name "iris-prod-vm-rpc" \
  --restore-points \
  --query "restorePoints[].{name:name, time:timeCreated, consistency:consistencyMode}" \
  -o table
```

### Create managed disks from a recovery point

Each recovery point contains disk restore point references. Extract them and create managed disks:

```bash
#!/usr/bin/env bash
# restore_from_recovery_point.sh

SUBSCRIPTION="00000000-0000-0000-0000-000000000000"
PROD_RG="iris-prod-rg"
COLLECTION="iris-prod-vm-rpc"
RP_NAME="rp-20260223120000"
TEST_RG="iris-test-rg"
LOCATION="eastus2"

# Get the recovery point's disk restore point IDs
DISK_RPS=$(az restore-point show \
  --subscription "$SUBSCRIPTION" \
  --resource-group "$PROD_RG" \
  --collection-name "$COLLECTION" \
  --name "$RP_NAME" \
  --query "sourceMetadata.storageProfile.dataDisks[].{name:name, diskRestorePoint:diskRestorePoint.id}" \
  -o json)

echo "Found $(echo "$DISK_RPS" | jq length) data disk restore points."

# Create a managed disk from each disk restore point
echo "$DISK_RPS" | jq -c '.[]' | while read -r entry; do
  DISK_NAME=$(echo "$entry" | jq -r '.name')
  DRP_ID=$(echo "$entry" | jq -r '.diskRestorePoint')
  NEW_DISK="iris-test-${DISK_NAME}"

  echo "Creating $NEW_DISK from disk restore point..."
